...
WaxlampFifoNetwork::WaxlampFifoNetwork()
  : MRecordRepeater(new Repeater<MTR::Record>),
    cacheGrouper(new AccessGrouper(Point(500,500), 100, 20, 300, 1.0, 1.0, true)) // TODO(choudhury): needs to become a CacheGrouper.
{}
...
std::vector<Widget::ptr> WaxlampFifoNetwork::addRegion(MTR::addr_t base, MTR::size_t size, MTR::size_t type){
  ...
  for(unsigned i=0; i<(limit - base) / type; i++){
    ...
    cacheGrouper->getAllGrouper()->addWidget(w);
  }

  cacheGrouper->getAllGrouper()->marshal();
